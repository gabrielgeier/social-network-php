...
<?php
	...
	$userId = $_GET['otherId'];
	$firstname = $_GET['otherFirst'];
	
	$query = "SELECT active FROM usr WHERE id=$userId";
	...
	if($checkQueryResult > 0){
		...
		echo "<h1>". $firstname." tem: ".$checkQueryResult. " livros(s)!</h1>";
		...
		while($data = pg_fetch_assoc($result)){
			...
			echo "<a id='book' href='../logged/index.php?book_info_viewr&bookId=".$data['book_id']. "'><img class='image-box'";
		...
		echo "<h1 class='no-results'>". $firstname." não tem livros cadastrados!</h1>";
	}